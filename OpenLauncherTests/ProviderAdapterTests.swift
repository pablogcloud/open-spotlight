...
final class ProviderAdapterTests: XCTestCase {
    func testClaudeParsesDeltaCompletionAndRateLimit() {
        ...
        let delta = adapter.parse(line: #"{"type":"stream_event","event":{"type":"content_block_delta","delta":{"type":"text_delta","text":"Ready"}}}"#)
        let completion = adapter.parse(line: #"{"type":"result","subtype":"success","is_error":false}"#)
        let rateLimit = adapter.parse(line: #"{"type":"assistant","error":"rate_limit","message":{"content":[{"type":"text","text":"Monthly spend limit reached"}]}}"#)

        XCTAssertEqual(delta, [.textDelta("Ready")])
        XCTAssertEqual(completion, [.completed(.init(exitCode: 0))])
        guard case let .failed(failure) = rateLimit.first else {
    ...
    func testCodexParsesAgentMessageAndIgnoresNonterminalWarningItem() {
        ...
        let warning = adapter.parse(line: #"{"type":"item.completed","item":{"type":"error","message":"warning"}}"#)
        let message = adapter.parse(line: #"{"type":"item.completed","item":{"type":"agent_message","text":"READY"}}"#)
    ...
    func testInvocationsPassPromptAsOneArgumentAndExposeEffectivePermissions() throws {
        ...
        XCTAssertNotEqual(grokInvocation.environment?["HOME"], FileManager.default.homeDirectoryForCurrentUser.path)
        XCTAssertEqual(
