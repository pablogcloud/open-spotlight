...
final class LauncherSuggestionTests: XCTestCase {
    func testCoordinatorRanksNativeAndIndexedResultsAheadOfPromptAndAsk() async {
        ...
        let metadata = StubMetadataSearch(values: [LauncherSuggestion(
            id: "metadata:numbers",
            kind: .application,
            title: "Numbers",
        ...
        let localIndex = StubLocalIndex(references: [LocalSearchReference(
            id: "budget:0",
            fileURL: indexedURL,
            title: "Budget",
        ...
        let values = await coordinator.suggestions(for: LauncherSuggestionRequest(
            query: "budget",
            provider: .codex,
