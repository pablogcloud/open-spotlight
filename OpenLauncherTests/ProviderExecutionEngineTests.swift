...
final class ProviderExecutionEngineTests: XCTestCase {
  ...
  func testClaudeStructuredErrorSequenceEmitsOneTerminalEvent() async throws {
    let runner = FixtureProcessRunner(events: [
      .standardOutput(
        Data(
          (#"{"type":"assistant","error":"rate_limit","message":{"content":[{"type":"text","text":"Limit reached"}]}}"#
            + "\n").utf8)),
      .standardOutput(
        Data(
          (#"{"type":"result","is_error":true,"result":"Limit reached","terminal_reason":"api_error"}"#
            + "\n").utf8)),
      .terminated(exitCode: 0, reason: .exit),
    ])
    ...
    guard case .failed(let failure) = events.last else {
      return XCTFail("Expected terminal failure")
    }
  ...
  func testProbeUsesFakeExecutableAndReportsVersion() async {
    ...
    guard case .available(let version, let executable) = descriptor.status else {
  ...
  func testProbeReportsAuthenticationRequiredWhenLoginStatusFails() async {
    ...
    guard case .authenticationRequired(let executableURL) = descriptor.status else {
  ...
  func testNonzeroExitOverridesStructuredSuccess() async throws {
    ...
    guard case .failed(let failure) = terminal else {
      return XCTFail("Expected nonzero exit failure")
    }
    XCTAssertEqual(failure.providerCode, "exit_1")
