...
struct LauncherView: View {
    ...
    private var surfaceStack: some View {
        VStack(spacing: 10) {
            ...
            if model.preferredSurfaceHeight > LauncherMetrics.controlSize {
                resultSurface
                    .transition(reducesMotion ? .opacity : .opacity.combined(with: .scale(scale: 0.985, anchor: .top)))
            }
        }
    }
    ...
    private var topBarContent: some View {
        ZStack(alignment: .leading) {
            ...
            trailingControl
                ...
                .offset(x: providerOffset + (showsProviderContent || reducesMotion ? 0 : -providerControlSize / 2))
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            if let file = model.selectedFile {
                Text(file.url.lastPathComponent)
                    .font(.system(size: 16, weight: .medium))
                    .lineLimit(1)
                Text("\(file.contents.count.formatted()) characters, \(ByteCountFormatter.string(fromByteCount: Int64(file.byteCount), countStyle: .file))")
                    .font(.system(size: 14))
                    .foregroundStyle(.secondary)
            ...
            if model.isProviderSetupInProgress {
                ...
                HStack {
                    if model.selectedProviderNeedsSetup {
                        Button("Connect \(model.selectedProvider.displayName)", action: model.requestProviderSetup)
                    } else {
    ...
    private var dropletPinchAnimation: Animation {
        reducesMotion ? .easeOut(duration: 0.08) : .spring(response: 0.25, dampingFraction: 0.82, blendDuration: 0.03)
    }
...
private extension View {
    ...
    func spotlightGlass<S: Shape>(
        ...
        if #available(macOS 26.0, *) {
            glassEffect(
                .regular
                    .tint(Color(nsColor: .windowBackgroundColor).opacity(
    ...
    @ViewBuilder
    func glassIdentity(_ id: String, namespace: Namespace.ID) -> some View {
        if #available(macOS 26.0, *) {
    ...
    @ViewBuilder
    func materializingGlassIdentity(_ id: String, namespace: Namespace.ID) -> some View {
        if #available(macOS 26.0, *) {
            glassEffectID(id, in: namespace)
