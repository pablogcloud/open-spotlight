...
struct OpenSpotlightOnboardingView: View {
  ...
  private func changePage(to nextPage: Int) {
    withAnimation(
      reduceMotion ? .easeOut(duration: 0.08) : .spring(response: 0.4, dampingFraction: 0.88)
    ) {
      page = nextPage
    }
  }
  ...
  var body: some View {
    Form {
      ...
      Section("Launcher") {
        ...
        Toggle(
          "Reveal the provider control from the magnifier", isOn: $preferences.revealProviderOnHover
        )
      ...
      Section("Local Index") {
        ...
        ForEach(Array(model.indexProgress.recentErrors.prefix(3).enumerated()), id: \.offset) {
          _, issue in
          Text(issue)
            .font(.caption2)
            .foregroundStyle(.secondary)
        ...
        Text(
          "Only specific folders you add are indexed. This is an early text-only index and is not yet resumable across relaunches. Search stays on this Mac; file content is never sent to a provider unless you explicitly attach and confirm it."
        )
        .font(.caption)
        .foregroundStyle(.secondary)
...
extension View {
  @ViewBuilder
  fileprivate func spotlightOnboardingGlass() -> some View {
