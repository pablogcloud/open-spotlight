...
final class LauncherViewModel {
  ...
  init(
    ...
    providerDescriptors = Dictionary(
      uniqueKeysWithValues: ProviderIdentifier.allCases.map {
      ...
      } else if let previewState = ProcessInfo.processInfo.environment[
        "OPEN_LAUNCHER_PREVIEW_STATE"]
      {
        prompt = "Reply with a concise status update"
        response =
          "The three provider adapters are connected and responding through their installed CLIs."
        switch previewState {
  ...
  func submitToProvider() {
    ...
    Task { [suggestionCoordinator, selectedProvider] in
      await suggestionCoordinator.recordSelection(
        LauncherSuggestion(
  ...
  func confirmFileAndSubmit() {
    ...
    do {
      ...
      let context = try ConfirmedFileContext(
        disclosure: disclosure, contents: selectedFile.contents)
  ...
  func moveSuggestionSelection(by delta: Int) {
    ...
    if suggestions.indices.contains(selectedSuggestionIndex) {
      selectedSuggestionIndex =
        (selectedSuggestionIndex + delta + suggestions.count) % suggestions.count
    } else {
  ...
  private func scheduleSuggestions(immediate: Bool = false) {
    guard suggestionsEnabled, runState != .streaming, runState != .awaitingDisclosure else {
      return
    }
    suggestionGeneration += 1
  ...
  private func migrateLegacyIndexIfNeeded() async {
    ...
    do {
      ...
      indexedChunkCount = 0
      indexWarning =
        if let result, let backupURL = result.backupURL {
  ...
  private func consume(_ request: ProviderRequest, generation: Int) async {
    do {
      ...
      for try await event in session.events {
        ...
        switch event {
        case .state(let state): runState = state
        case .textDelta(let text): response += text
...
extension ProviderExecutionError: LocalizedError {
  var errorDescription: String? {
    ...
    case .executableNotFound(let provider):
      "\(provider.displayName) is not installed or executable."
