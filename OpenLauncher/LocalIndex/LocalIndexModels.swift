...
enum LocalIndexError: Error, Equatable, LocalizedError, Sendable {
    ...
    var errorDescription: String? {
        switch self {
        case let .unapprovedURL(url): "The file is outside the approved index folders: \(url.path)"
        case let .forbiddenRoot(url, reason): "The folder cannot be indexed: \(url.path). \(reason)"
        case let .unsupportedFileType(extensionName): "The file type .\(extensionName) is not supported by the local index."
        case let .invalidDatabase(message): "The local index database failed: \(message)"
...
enum LocalIndexRootPolicy {
    ...
    static func rejectionReason(for candidate: URL) -> String? {
        ...
        guard url.path != "/" else { return "Choose specific folders instead of the whole startup volume." }
        ...
        if url.path == "/Volumes" || (components.count == 3 && components.dropFirst().first == "Volumes") {
