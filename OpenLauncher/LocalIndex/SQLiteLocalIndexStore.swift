...
actor SQLiteLocalIndexStore {
  ...
  func statistics() throws -> (documents: Int, chunks: Int) {
    (
      try scalarCount("SELECT COUNT(*) FROM documents"),
      try scalarCount("SELECT COUNT(*) FROM chunks")
    )
  }
  ...
  func backup(to destinationURL: URL) throws {
    ...
    guard
      ...
      let message =
        destination.map { String(cString: sqlite3_errmsg($0)) } ?? "Unable to create index backup"
