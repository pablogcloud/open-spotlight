...
actor LauncherSuggestionCoordinator: LauncherSuggestionCoordinating {
  ...
  func suggestions(for request: LauncherSuggestionRequest) async -> [LauncherSuggestion] {
    ...
    async let localValues = localSuggestions(
      query: query, roots: request.indexedRoots, limit: sourceLimit)
    ...
    let askSuggestion: LauncherSuggestion? =
      query.isEmpty
      ? nil
      : LauncherSuggestion(
  ...
  private func localSuggestions(query: String, roots: [URL], limit: Int) async
    -> [LauncherSuggestion]
  {
  ...
  private static func promptSuggestions(query: String, provider: ProviderIdentifier)
    -> [LauncherSuggestion]
  {
  ...
  private static func deduplicationKey(_ action: LauncherSuggestionAction) -> String {
    switch action {
    case .open(let url): "open:\(url.standardizedFileURL.path)"
    case .ask(let query): "ask:\(query.lowercased())"
    case .fillPrompt(let prompt): "fill:\(prompt.lowercased())"
