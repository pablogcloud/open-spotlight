...
actor SpotlightMetadataSuggestionSource: SpotlightMetadataSearching {
    func search(_ query: String, limit: Int) -> [LauncherSuggestion] {
        ...
        MDQuerySetSearchScope(
            metadataQuery,
            [kMDQueryScopeHome, "/Applications" as CFString, "/System/Applications" as CFString] as CFArray,
            0
        )
        guard MDQueryExecute(metadataQuery, CFOptionFlags(kMDQuerySynchronous.rawValue)) else { return [] }
        ...
        for index in 0..<resultCount {
            ...
            let kind: LauncherSuggestionKind = isApplication ? .application : (isDirectory ? .folder : .file)
            let subtitle = isApplication
                ? "Application"
