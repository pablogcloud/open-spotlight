...
actor LauncherHistoryStore {
    ...
    func record(_ suggestion: LauncherSuggestion) {
        ...
        case let .open(url):
            entry = LauncherHistoryEntry(kind: .open, value: url.path, title: suggestion.title, usedAt: .now)
        case let .ask(query):
    ...
    func suggestions(matching query: String, provider: ProviderIdentifier, limit: Int) -> [LauncherSuggestion] {
