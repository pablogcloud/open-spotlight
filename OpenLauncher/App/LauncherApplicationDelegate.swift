...
final class LauncherApplicationDelegate: NSObject, NSApplicationDelegate {
  ...
  private func installStatusItem() {
    ...
    item.button?.image = NSImage(
      systemSymbolName: "magnifyingglass", accessibilityDescription: "Open Spotlight")

    let menu = NSMenu()
    let open = NSMenuItem(
      title: "Open Spotlight", action: #selector(showLauncher), keyEquivalent: "")
    open.target = self
    menu.addItem(open)
    ...
    for shortcut in LauncherShortcut.allCases {
      let menuItem = NSMenuItem(
        title: shortcut.title, action: #selector(selectShortcut), keyEquivalent: "")
