...
final class LauncherPreferences {
    ...
    var defaultProvider: ProviderIdentifier { didSet { persist(defaultProvider.rawValue, key: Key.defaultProvider) } }
    var placement: LauncherPlacement { didSet { persist(placement.rawValue, key: Key.placement) } }
    var resultSize: LauncherResultSize { didSet { persist(resultSize.rawValue, key: Key.resultSize) } }
    var glassContrast: LauncherGlassContrast { didSet { persist(glassContrast.rawValue, key: Key.glassContrast) } }
    var clearQueryOnOpen: Bool { didSet { persist(clearQueryOnOpen, key: Key.clearQueryOnOpen) } }
    var revealProviderOnHover: Bool { didSet { persist(revealProviderOnHover, key: Key.revealProviderOnHover) } }
    var dismissOnOutsideClick: Bool { didSet { persist(dismissOnOutsideClick, key: Key.dismissOnOutsideClick) } }
    var reduceMotion: Bool { didSet { persist(reduceMotion, key: Key.reduceMotion) } }
    var indexedFolderPaths: [String] { didSet { persist(indexedFolderPaths, key: Key.indexedFolderPaths) } }
    private(set) var pendingLegacyIndexQuarantinePaths: [String]

    init(defaults: UserDefaults = .standard) {
        self.defaults = defaults
        defaultProvider = defaults.string(forKey: Key.defaultProvider)
        ...
        resultSize = defaults.string(forKey: Key.resultSize)
            .flatMap(LauncherResultSize.init(rawValue:)) ?? .comfortable
        glassContrast = defaults.string(forKey: Key.glassContrast)
    ...
    var indexedFolders: [URL] {
        LocalIndexRootPolicy.allowedRoots(from: indexedFolderPaths.map {
    ...
    private static func partitionIndexPaths(_ paths: [String]) -> (allowed: [String], forbidden: [String]) {
