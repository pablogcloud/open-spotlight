...
struct CodexAdapter: CLIProviderAdapter {
  ...
  let invocationDisclosure = ProviderInvocationDisclosure(
    ...
    residualAccess:
      "Codex may inspect files readable inside its CLI sandbox; writes and network access are blocked by the launcher-selected sandbox."
  )
  ...
  func parse(line: String) -> [NormalizedStreamEvent] {
    ...
    if type == "turn.failed" {
      let message =
        ProviderJSON.string("error", "message", in: object)
