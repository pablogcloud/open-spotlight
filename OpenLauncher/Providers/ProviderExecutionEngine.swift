...
struct ProviderExecutionEngine: Sendable {
  ...
  func probe(_ identifier: ProviderIdentifier) async -> ProviderDescriptor {
    ...
    do {
      ...
      for try await event in process.events {
        switch event {
        case .standardOutput(let data), .standardError(let data): output.append(data)
        case .terminated(let code, _): exitCode = code
        }
      }
  ...
  private func isAuthenticated(
    ...
    do {
      ...
      var exitCode: Int32 = -1
      for try await event in process.events {
        if case .terminated(let code, _) = event { exitCode = code }
      }
      return exitCode == 0
    } catch {
      return false
      ...
      let task = Task {
        ...
        do {
          for try await event in process.events {
            ...
            case .terminated(let exitCode, _):
              ...
              if state.isCancelled {
                ...
                if sawOutputLine {
                  continuation.yield(
                    .failed(Self.malformedOutputFailure(identifier: request.provider)))
                } else {
